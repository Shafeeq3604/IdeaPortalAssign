...
async function main(): Promise<void> {
  ...
  // Criteria, profiles/weights, and model routes — shared with `seed-config.ts` so a
  // fix to one keeps the other in sync rather than drifting apart (see that file for
  // why it also needs to run without everything below this line).
